...
namespace Quantum.Pacman {
    public unsafe class PacmanSystem : SystemMainThreadFilter<PacmanSystem.Filter>, ISignalOnPacmanScored, ISignalOnEntityEaten,
        ISignalOnPacmanKilled, ISignalOnPacmanRespawned, ISignalOnPowerPelletStart, ISignalOnPowerPelletEnd {
        ...
        public override void Update(Frame f, ref Filter filter) {
            ...
            var asset = f.FindAsset(pacman->Asset);
            ...
            if (pacman->IsDead) {
                FP bombUseCutoff = asset.RespawnTime - asset.BombUseTimeAfterDeath;
                if (pacman->RespawnTimer >= bombUseCutoff) {
                    if (pacman->Bombs > 0 && input.Bomb.WasPressed) {
                        // Allow bomb use ~100ms after dying
                        UseBomb(f, ref filter, asset);
                        pacman->RespawnTimer = 0;
                        pacman->IsDead = false;
                        mover->IsLocked = false;
                    }
                } else {
                    if (pacman->RespawnTimer + f.DeltaTime >= bombUseCutoff) {
            ...
            // Player movement
            if (!mover->IsLocked && mover->FreezeTime == 0 && input.TargetDirection != -1 && mover->Direction != input.TargetDirection) {
            ...
            if (pacman->HasPowerPellet) {
                ...
                for (int i = 0; i < hits.Count; i++) {
                    ...
                    if (!otherPac->HasPowerPellet && !otherPac->Invincible) {
                        // Pac1 eaten Pac2
                        f.Signals.OnEntityEaten(entity, hit.Entity);
        ...
        public static void UseBomb(Frame f, ref Filter filter, PacmanAsset asset) {
            var entity = filter.Entity;
            var pacman = filter.Pacman;
            var transform = filter.Transform;
            var mover = filter.Mover;
            ...
            FP travelTime = asset.BombBaseTravelTime + asset.BombDistanceTravelTime * FPVector2.Distance(target, transform->Position);
        ...
        public void OnPowerPelletStart(Frame f, EntityRef entity) {
            ...
            while (filter.NextUnsafe(out _, out var pacman2, out var mover2)) {
                if (!pacman2->HasPowerPellet) {
                    mover2->SpeedMultiplier = f.FindAsset(pacman2->Asset).OtherPacmanPowerPelletSlowdownMultiplier;
                }
            }
        }
        ...
        public void OnPacmanRespawned(Frame f, EntityRef entity) {
            ...
            pac->Invincibility = f.FindAsset(pac->Asset).RespawnInvincibilityTime;
        ...
        public void OnPacmanKilled(Frame f, EntityRef entity) {
            if (f.Unsafe.TryGetPointer(entity, out PacmanPlayer* pac)) {
                pac->IsDead = true;
                pac->RespawnTimer = f.FindAsset(pac->Asset).RespawnTime;
            }
        ...
        public void OnEntityEaten(Frame f, EntityRef pacmanEntity, EntityRef other) {
            ...
            pacman->PowerPelletTimer = FPMath.Min(pacman->PowerPelletTimer + asset.GhostEatFreezeTime, pacman->PowerPelletFullTimer);

            // Freeze
            if (f.Unsafe.TryGetPointer(pacmanEntity, out GridMover* pacmanMover)) {
                pacman->TemporaryInvincibility = asset.GhostEatAdditionalInvincibilityTime;
                pacmanMover->FreezeTime = asset.GhostEatFreezeTime;
            }
            if (f.Unsafe.TryGetPointer(other, out GridMover* otherMover)) {
                otherMover->FreezeTime = asset.GhostEatFreezeTime;
            }

            f.Signals.OnPacmanScored(pacmanEntity, points);
            f.Events.EntityEaten(pacmanEntity, other, pacman->GhostCombo, points);
        }
